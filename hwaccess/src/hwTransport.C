...
int HwTransportIntf::getCfam(const char* path, uint32_t addr, uint32_t& val)
{
    int fd = -1;
    int ret = kernel_fsi_probe(fd);

    std::cout << "DEVICE_PATH=" << path << std::endl;
...
int HwTransportIntf::putCfam(const char* path, uint32_t addr, uint32_t val)
{}
