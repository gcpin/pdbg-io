...
int HwTransportInterface::getCfam(std::string& path, uint32_t addr, uint32_t& val)
{
    int fd = -1;
    int ret = kernel_fsi_probe(fd);

    std::cout << "PHYS_DEV_PATH=" << path.c_str() << std::endl;
...
int HwTransportInterface::putCfam(std::string& path, uint32_t addr, uint32_t val)
{}
